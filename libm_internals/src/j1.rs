...
fn common(ix: u32, x: f64, y1: i32, sign: u32) -> f64 {
    /*
...
fn pone(x: f64) -> f64 {
    ...
    return if (ix >= 0x40200000) {
        pone_compute(pr8, ps8, x)
    } else if (ix >= 0x40122E8B) {
        pone_compute(pr5, ps5, x)
    } else if (ix >= 0x4006DB6D) {
        pone_compute(pr3, ps3, x)
    } else
    /*ix >= 0x40000000*/
    {
        pone_compute(pr2, ps2, x)
    };
}
#[inline]
fn pone_compute(p: [f64; 6], q: [f64; 5], x: f64) -> f64 {
    let z = 1.0 / (x * x);
...
fn qone(x: f64) -> f64 {
    let ix = get_high_word(x) & 0x7fffffff;
    return if (ix >= 0x40200000) {
        qone_compute(qr8, qs8, x)
    } else if (ix >= 0x40122E8B) {
        qone_compute(qr5, qs5, x)
    } else if (ix >= 0x4006DB6D) {
        qone_compute(qr3, qs3, x)
    } else
    /*ix >= 0x40000000*/
    {
